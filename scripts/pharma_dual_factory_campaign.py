#!/usr/bin/env python3
"""
DESCARTES-PHARMA: Dual Factory Campaign -- Training Configuration Evolution
============================================================================
...
C1 Factory: Fast mechanistic probes (Ridge dR2 on 4 catalytic features).
...
print("=" * 85)
print("DESCARTES-PHARMA: Dual Factory Campaign")
...
def fast_probe_catalytic(embeddings):
    """C1 Fast Probe: Ridge dR2 for 4 catalytic features only. ~1s per model."""
    results = {}
    rng_emb = np.random.default_rng(42)
    rand_emb = rng_emb.standard_normal(embeddings.shape)
    ...
    for fi, fname in zip(CATALYTIC_IDX, CATALYTIC_FEATURES):
        ...
        if np.std(t) < 1e-10:
            results[fname] = 0.0
            continue
        st = cross_val_score(Ridge(alpha=1.0), embeddings, t,
                             cv=kf, scoring='r2')
        sr = cross_val_score(Ridge(alpha=1.0), rand_emb[:len(t)], t,
                             cv=kf, scoring='r2')
        results[fname] = float(np.mean(st) - np.mean(sr))
    return results


def compute_fitness(auc, catalytic_results, pocket_results=None):
    """Section 7.5 fitness function.
    ...
    n_cat = sum(1 for v in catalytic_results.values() if v > 0.05)

    if pocket_results is not None:
        n_pock = sum(1 for v in pocket_results.values() if v > 0.05)
    else:
        n_pock = min(n_cat, 2)

    fitness = 0.3 * (auc / 0.95) + 0.5 * (n_cat / 4.0) + 0.2 * (n_pock / 4.0)
    return fitness
    ...
    if LLM_AVAILABLE:
        try:
            ...
            prompt = (
                ...
                "  fitness = 0.3*(AUC/0.95) + 0.5*(n_catalytic_encoded/4) "
                "+ 0.2*(n_pocket_encoded/4)\n"
                "where n_catalytic_encoded counts how many of [dist_asp32, "
                "dist_asp228, hbond_catalytic, catalytic_score] are encoded "
                "(Ridge dR2 > 0.05).\n\n"
...
t_g = time.time()
print("\n" + "=" * 85)
print("PHASE G: 40-Round Dual Factory Campaign")
print("=" * 85)
...
for round_num in range(1, N_ROUNDS + 1):
    ...
    # --- C1 Fast Probe (catalytic only) ---
    cat_results = fast_probe_catalytic(embeddings)
    n_cat = sum(1 for v in cat_results.values() if v > 0.05)
    ...
    print(f"  Round {round_num:2d}: {genome.genome_id:<32} | "
          f"AUC={auc:.2f} | cat={n_cat}/4 | "
          f"fitness={fitness:.4f} | {best_flag}  "
          f"({phase}, {time.time() - round_t:.1f}s)")

print(f"\n  40-round campaign completed in {time.time() - t_g:.1f}s")
print(f"  Best genome: {best_genome.genome_id}")
print(f"  Best fitness: {best_fitness:.4f}")
print(f"  Best AUC: {best_auc:.4f}")
print(f"  Best catalytic: {best_cat_results}")
...
# Random baseline embeddings
rand_model = ConcatModelMultiHead(
    n_aux_outputs=best_genome.n_aux_outputs(),
    use_information_bottleneck=best_genome.information_bottleneck
).to(device)
rand_model.eval()
emb_random = []
with torch.no_grad():
    for batch in dtsl:
        batch = batch.to(device)
        _, emb, _, _ = rand_model(batch, pocket_summary_tensor,
                                   return_embedding=True)
        emb_random.append(emb.cpu().numpy())
emb_random = np.concatenate(emb_random)
...
for seed in range(N_SEEDS):
    ...
    rand_emb_s = rng_s.standard_normal(s_emb.shape)
...
for rank, (g, a, cr, f) in enumerate(sorted_all, 1):
    n_cat = sum(1 for v in cr.values() if v > 0.05)
    desc = g.short_desc()
    if len(desc) > 40:
        desc = desc[:37] + "..."
...
for rank, (g, a, cr, f) in enumerate(sorted_all[:5], 1):
    ...
    print(f"    Catalytic: "
          f"{', '.join(f'{k}={v:.3f}' for k, v in cr.items())}")
...
if baseline_result:
    ...
    bl_cat = sum(1 for v in bl_cr.values() if v > 0.05)
    print(f"\n  BASELINE COMPARISON:")
